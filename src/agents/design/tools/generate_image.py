from pathlib import Path

from google import genai
import os
from dotenv import load_dotenv
from google.adk.tools import tool
from src.agents.design.utils.artifact_utils import load_prompt, generate_output_path

load_dotenv()

@tool(
    name="generate_image",
    description="Generate a new image for an asset using design instructions."
)
def generate_image(name: str, design_instructions: str):
    client = genai.Client(api_key=os.getenv("GOOGLE_API_KEY"))

    nano_create_prompt = load_prompt("nano_create_prompt.md")
    final_prompt = f"""{nano_create_prompt}

    {design_instructions}
    """
    print(final_prompt)
    response = client.models.generate_content(
        model="gemini-2.5-flash-image",
        contents=[final_prompt],
    )

    for part in response.parts:
        if part.text is not None:
            print(part.text)
        elif part.inline_data is not None:
            image = part.as_image()
            output_path = generate_output_path(name)
            print(f" Image will be saved to {output_path}")
            image.save(str(output_path))

#
# PROJECT_ROOT = Path(__file__).resolve().parent.parent
# PROMPTS_DIR = PROJECT_ROOT / "prompts"
#
# path = PROMPTS_DIR / "sample_design_create_prompt.md"
# generate_image("home_bakery",path.read_text(encoding="utf-8"))